...
def test_freq_index():
    ...
    assert data_handle.frequency.freq_index(freq, nfft, fs) == expected_index


def test_frequency_class():
    kwargs = {"sampling_rate": 16000, "nfft": 1024, "min_freq": 0, "max_freq": 8000}
    frequency = data_handle.frequency.Frequency(**kwargs)
    ...
    assert frequency.min_freq_index == data_handle.frequency.freq_index(
        freq=frequency.min_freq, nfft=frequency.nfft, fs=frequency.fs
    )
    assert frequency.max_freq_index == data_handle.frequency.freq_index(
